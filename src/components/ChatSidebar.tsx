      ...
      <div className="sm:hidden fixed bottom-0 inset-x-0 z-[90] bg-[var(--bg-secondary)] border-t border-[var(--border)]">
        {isOpen ? (
          ...
          <div className="h-[60vh] flex flex-col">
            ...
            <MessageList messages={messages} scrollRef={scrollRef} isLoading={isLoading} onAcceptChange={onAcceptChange} onRejectChange={onRejectChange} />
...
function MessageList({ messages, scrollRef, isLoading, onAcceptChange, onRejectChange }: {
  messages: ChatMsg[]; scrollRef: React.RefObject<HTMLDivElement>; isLoading: boolean;
  onAcceptChange: (msgId: string, idx: number) => void;
  onRejectChange: (msgId: string, idx: number) => void;
}) {
  return (
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <div className="min-w-0">
                            <span className="text-[var(--text-secondary)]">
                              {sc.action === 'append' ? 'Add to' : 'Update'}{' '}
                            </span>
