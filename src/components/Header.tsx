...
export default function Header({ onRefresh }: HeaderProps) {
  ...
  const isSearchPage = pathname?.includes('/search/') && !pathname?.endsWith('/settings');
  const isProjectPage = pathname?.startsWith('/projects/') && !pathname?.includes('/names/search/');
    ...
    <header className="sticky top-0 z-50 backdrop-blur-xl bg-[#0a0a0f]/80 border-b border-[var(--border)]">
      <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          {isSearchPage && onRefresh && (
            <>
              <div className="w-px h-4 bg-[var(--border)]" />
              <button
                onClick={onRefresh}
                className="text-sm text-[var(--text-secondary)] hover:text-[var(--accent)] transition-colors flex items-center gap-1.5"
                title="Clear all names and chat, keep settings"
              >
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Settings — only on project/search pages */}
          {(isProjectPage || isSearchPage) && (
            <button
              onClick={() => {
